...
public interface MonitorRepository extends JpaRepository<Monitor, Long> {
    ...
    @Query("SELECT m FROM Monitor m WHERE m.id = :id AND m.ownerId = :ownerId")
    ...
    @Query("SELECT m FROM Monitor m WHERE m.enabled = true AND (m.nextPingAt <= :now OR m.nextPingAt IS NULL) ORDER BY m.nextPingAt ASC NULLS FIRST")
